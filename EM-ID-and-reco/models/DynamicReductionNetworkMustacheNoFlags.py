...
class DynamicReductionNetworkMustacheNoFlags(nn.Module):
    '''
    ...
    '''
    def __init__(self, input_dim_xECAL=4, input_dim_fECAL = 4, 
            hidden_dim=64, 
            output_dim=6, k=16, aggr='add', 
            loop=True, pool='max',
            agg_layers=6, mp_layers=3, in_layers=4, out_layers=2,
            graph_features = 1):
        super(DynamicReductionNetworkMustacheNoFlags, self).__init__()
        ...
        #construct inputnet
        in_layers_ECAL_l = []
        in_layers_ECAL_l += [nn.Linear(input_dim_xECAL, hidden_dim),
                nn.ELU()]
    ...
    def forward(self, xECAL: Tensor, graph_x: Tensor, batch: OptTensor) -> Tensor:
        '''
        ...
        for i, edgeconv in enumerate(self.agg_layers):
            knn = knn_graph(xECAL, self.k, batch, loop=self.loop, flow=edgeconv.flow)
            edge_index = to_undirected(knn)
            xECAL = edgeconv(xECAL, edge_index)

            weight = normalized_cut_2d(edge_index, xECAL)
            cluster = graclus_cluster(edge_index[0], edge_index[1], weight, xECAL.size(0))

            if i == nAgg - 1:
                xECAL, batch = aggr_pool_x(cluster, xECAL, batch, self.aggr_type)
            else:
                xECAL, batch = aggr_pool(cluster, xECAL, batch, self.aggr_type)


        # this xforms to batch-per-row so no need to return batch
        xECAL = global_pool_aggr(xECAL, batch, self.aggr_type)

        xECAL = torch.cat((xECAL, torch.unsqueeze(graph_x,1)), 1)

        xECAL = self.output(xECAL).squeeze(-1)
