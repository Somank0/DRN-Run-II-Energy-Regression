...
class DynamicReductionNetworkMustache(nn.Module):
    '''
    ...
    '''
    def __init__(self, input_dim_xECAL=5, input_dim_fECAL = 4, 
            hidden_dim=64, 
            output_dim=6, k=16, aggr='add', 
            loop=True, pool='max',
            agg_layers=6, mp_layers=3, in_layers=4, out_layers=2,
            graph_features = 1):
        super(DynamicReductionNetworkMustache, self).__init__()
        ...
        self.flag_embed = nn.Embedding(8, 3)

        #construct inputnet
        in_layers_ECAL_l = []
        in_layers_ECAL_l += [nn.Linear(input_dim_xECAL + 4, hidden_dim),
                nn.ELU()]
    ...
    def forward(self, xECAL: Tensor, fECAL: Tensor, gainECAL: Tensor, graph_x: Tensor, batch: OptTensor) -> Tensor:
        '''
        Push the batch 'data' through the network
        '''
        fECAL = self.flag_embed(fECAL)
        gainECAL = self.gain_embed(gainECAL)
        ECAL = torch.cat( (xECAL, fECAL, gainECAL), 1)
        ...
        for i, edgeconv in enumerate(self.agg_layers):
            knn = knn_graph(ECAL, self.k, batch, loop=self.loop, flow=edgeconv.flow)
            edge_index = to_undirected(knn)
            ECAL = edgeconv(ECAL, edge_index)

            weight = normalized_cut_2d(edge_index, ECAL)
            cluster = graclus_cluster(edge_index[0], edge_index[1], weight, ECAL.size(0))

            if i == nAgg - 1:
                ECAL, batch = aggr_pool_x(cluster, ECAL, batch, self.aggr_type)
            else:
                ECAL, batch = aggr_pool(cluster, ECAL, batch, self.aggr_type)

        # this xforms to batch-per-row so no need to return batch
        ECAL = global_pool_aggr(ECAL, batch, self.aggr_type)

        ECAL = torch.cat((ECAL, torch.unsqueeze(graph_x,1)), 1)

        ECAL = self.output(ECAL).squeeze(-1)
